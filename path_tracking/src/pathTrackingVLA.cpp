...
class PathTrackingNode : public rclcpp::Node
{
    private:
        ...
        std::string uart_device_{uart_ascii_cmd::kDefaultDevice};
        unsigned long uart_baud_{uart_ascii_cmd::kDefaultBaud};
    ...
    public:
        PathTrackingNode() : Node("pathTracking")
        {
            ...
            this->declare_parameter<std::string>("uart_device", uart_ascii_cmd::kDefaultDevice);
            this->declare_parameter<int>("uart_baud", static_cast<int>(uart_ascii_cmd::kDefaultBaud));
            ...
            {
                std::string open_err;
                uart_fd_ = uart_ascii_cmd::open_serial_ascii(uart_device_, uart_baud_, &open_err);
        ...
        void pathCallback(const nav_msgs::msg::Path::SharedPtr mpoints) 
        {
            ...
            if (cmd_len <= 0 || static_cast<size_t>(cmd_len) >= sizeof(cmd_line))
            {
                RCLCPP_WARN_THROTTLE(
                    ...
                    "UART: failed to format ASCII cmd (vx=%.3f yaw=%.3f)", axial_vel, turning_rate);
            }
            else if (uart_fd_ >= 0)
            {
                if (uart_ascii_cmd::write_all_fd(uart_fd_, cmd_line, static_cast<size_t>(cmd_len)))
                {
                    if (log_uart_cmd_)
                    {
                        if (!uart_first_tx_logged_)
                        {
                            ...
                            RCLCPP_INFO(
                                this->get_logger(),
                                "UART: first ASCII cmd sent (tests/uart_over_usb.py format). Further lines throttled.");
                        }
                        RCLCPP_INFO_THROTTLE(
                            this->get_logger(), *this->get_clock(), 500,
                            "Control(UART): %s", cmd_line);
                    }
                }
                else
                {
            ...
            else
            {
                ...
                RCLCPP_WARN_THROTTLE(
                    ...
                    "UART not transmitting (port not open). Last open error: %s | would send: %s",
                    reason, cmd_line);
            }
        }
