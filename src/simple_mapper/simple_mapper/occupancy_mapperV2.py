...
class OccupancyMapper(Node):
    ...
    def __init__(self):
        ...
        # Time-matched pose buffer (~1 seconds at 240 Hz Vicon)
        self.pose_buffer: deque = deque(maxlen=200)
    ...
    # -----------------------------------------------------------------------
    # Time-matched pose lookup
    # -----------------------------------------------------------------------

    def _get_pose_at_time_sec(self, scan_t: float) -> PoseStamped | None:

        MAX_ALLOWED_DELTA_SEC = 0.025

        if len(self.pose_buffer) < 2:
            return None

        pose_list = list(self.pose_buffer)
        times = [p.header.stamp.sec + p.header.stamp.nanosec * 1e-9
                 for p in pose_list]

        # Find the two poses that bracket scan_t
        before   = None
        after    = None
        before_t = -float('inf')
        after_t  =  float('inf')
        ...
        for p, t in zip(pose_list, times):
            if t <= scan_t and t > before_t:
                before   = p
                before_t = t
            if t > scan_t and t < after_t:
                after   = p
                after_t = t

        # Fall back to nearest single pose if we can't bracket
        if before is None and after is None:
            return None
        if before is None:
            if abs(after_t - scan_t) < MAX_ALLOWED_DELTA_SEC:
                return after
            return None
        if after is None:
            if abs(scan_t - before_t) < MAX_ALLOWED_DELTA_SEC:
                return before
            return None

        # Check the bracket isn't too wide
        if (after_t - before_t) > MAX_ALLOWED_DELTA_SEC:
            self.get_logger().warn(
                f'Vicon bracket too wide: {(after_t - before_t)*1000:.1f} ms',
                throttle_duration_sec=2.0,
            )
            return before

        # Linear interpolation factor [0, 1]
        alpha = (scan_t - before_t) / (after_t - before_t)

        # Interpolate position
        interp = PoseStamped()
        interp.header.frame_id = before.header.frame_id

        b = before.pose.position
        a = after.pose.position
        interp.pose.position.x = b.x + alpha * (a.x - b.x)
        interp.pose.position.y = b.y + alpha * (a.y - b.y)
        interp.pose.position.z = b.z + alpha * (a.z - b.z)
        ...
        dot = sum(x * y for x, y in zip(q1, q2))
        ...
        # Normalise
        mag = math.sqrt(sum(x * x for x in qr))
        qr  = [x / mag for x in qr]
    ...
    # -----------------------------------------------------------------------
    # Scan callback — per-ray time-matched pose lookup
    # -----------------------------------------------------------------------
    ...
    def on_scan(self, scan: LaserScan):
        ...
        for i, r in enumerate(scan.ranges):
            ...
            # Each ray has its own capture time
            ray_t = (scan.header.stamp.sec
